import"./assets/themeBackground-e2f72e63.js";import{a as U,l as S,i as u}from"./assets/vendor-57c75b59.js";const N="https://your-energy.b.goit.study/api",k=12,A=10,p=U.create({baseURL:N,params:{}}),H=async(e,t=1)=>(await p.get("/filters",{params:{filter:e,page:t,limit:k}})).data,j=async(e=1)=>(await p.get("/filters",{params:{filter:"Muscles",page:e,limit:k}})).data,F=async()=>(await p.get("/quote")).data,Q=async({bodypart:e,muscles:t,equipment:a,search:s},n=1)=>{const c=s&&!e&&!t&&!a?"":s;return(await p.get("/exercises",{params:{bodypart:e||"",muscles:t||"",equipment:a||"",keyword:c||"",page:n,limit:A}})).data},P=async e=>(await p.get(`/exercises/${e}`)).data,K=async(e,t)=>(await p.patch(`/exercises/${e}/rating`,t)).data;function q(e){return e.map(({imgURL:t,filter:a,name:s})=>`<li class="card-exercises" data-body-exercise='${s}'>
...
</div >`}function re(){ce.classList.remove("is-hidden"),h.classList.add("is-hidden")}function ie(e){const t=e.target;if(!t.classList.contains("js-excercise-button"))return;const s=t.closest(".exercises-item").dataset.exerciseId;P(s).then(n=>{const c=oe(n);te.innerHTML=c}),ne.setAttribute("data-id",s),h.classList.remove("is-hidden"),x.style.overflow="hidden"}function B(e){e.key==="Escape"?(h.classList.add("is-hidden"),x.style.overflow="visible"):e.target.closest(".modal_window_default_content,.js-excercise-button")||(h.classList.add("is-hidden"),x.style.overflow="visible")}function le(){h.classList.add("is-hidden"),x.style.overflow="visible"}const R="quote",de=document.querySelector(".quote-text"),ue=document.querySelector(".quote-author");async function me(){try{const{author:e,quote:t}=await F(),a=new Date().toLocaleDateString();localStorage.setItem(R,JSON.stringify({author:e,quote:t,date:a}))}catch(e){console.error("Failed to fetch the quote:",e)}}function I(){const e=localStorage.getItem(R);return JSON.parse(e)}function E(){const e=I();de.textContent=e.quote,ue.textContent=e.author}async function pe(){const e=new Date().toLocaleDateString(),t=I();if(t&&t.date===e){E();return}await me(),E()}document.addEventListener("DOMContentLoaded",pe);
