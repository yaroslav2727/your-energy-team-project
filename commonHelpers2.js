import"./assets/themeBackground-59b3794d.js";import{a as D,i as m}from"./assets/vendor-96017409.js";const C="https://your-energy.b.goit.study/api",b=12,B=10,u=D.create({baseURL:C,params:{}}),I=async(e,t=1)=>(await u.get("/filters",{params:{filter:e,page:t,limit:b}})).data,R=async(e=1)=>(await u.get("/filters",{params:{filter:"Muscles",page:e,limit:b}})).data,O=async()=>(await u.get("/quote")).data,N=async({bodypart:e,muscles:t,equipment:c,search:s},n=1)=>{const a=s&&!e&&!t&&!c?"":s;return(await u.get("/exercises",{params:{bodypart:e||"",muscles:t||"",equipment:c||"",keyword:a||"",page:n,limit:B}})).data},A=async e=>(await u.get(`/exercises/${e}`)).data,U=async(e,t)=>(await u.patch(`/exercises/${e}/rating`,t)).data;function E(e){return e.map(({imgURL:t,filter:c,name:s})=>`<li class="card-exercises" data-body-exercise='${s}'>
  ...
  <div class="card-description">
   <p class="name-category">${s}</p>
	...
	</li>`).join("")}const d={element:document.querySelector(".loader"),create(){this.element.classList.add("loader-active")},destroy(){this.element.classList.remove("loader-active")}};let Q=1,S="muscles",v=null;const o=document.querySelector(".cards"),H=document.querySelector(".filter-list"),P=document.querySelector(".input-filter-exercises");H.addEventListener("click",K);o.addEventListener("click",x);P.addEventListener("input",z);d.create();R().then(e=>{const t=e.results;o.innerHTML=E(t),o.addEventListener("click",x)}).catch(e=>{console.error(e),m.show({position:"center",color:"red",message:"Oops! Something wrong!"})}).finally(()=>{d.destroy()});function K(e){if(e.target.nodeName!=="BUTTON")return;const t=e.target.dataset.name;document.querySelectorAll(".filter-btn").forEach(s=>s.classList.remove("active")),e.target.classList.add("active"),d.create(),I(t,Q).then(s=>{const n=s.results,{filter:a}=n[0];S=a.toLowerCase(),o.innerHTML=E(n),o.addEventListener("click",x)}).catch(s=>{console.error(s),m.show({position:"center",color:"red",message:"Oops! Something wrong!"})}).finally(()=>{d.destroy()})}function x(e){const t=e.target.closest(".card-exercises").dataset.bodyExercise,c={[S]:t,page:1};d.create(),N(c).then(s=>{const n=s.results;v=n,console.log(v),o.innerHTML=_(n)}).catch(s=>{console.error(s),m.show({position:"center",color:"red",message:"Oops! Something wrong!"})}).finally(()=>{d.destroy(),o.removeEventListener("click",x)})}function z(e){const t=e.target.value.trim().toLowerCase(),c=v.filter(s=>s.name.includes(t));o.innerHTML=_(c)}function J(){const e=document.querySelector(".js-menu-container"),t=document.querySelector(".js-open-menu"),c=document.querySelector(".js-close-menu"),s=document.body,n=()=>{const l=t.getAttribute("aria-expanded")==="true"||!1;t.setAttribute("aria-expanded",!l),e.classList.toggle("is-open"),s.classList.toggle("menu-open"),s.classList.contains("menu-open")?r():i()},a=()=>{e.classList.remove("is-open"),t.setAttribute("aria-expanded",!1),s.classList.remove("menu-open"),i()},r=()=>{s.style.overflow="hidden"},i=()=>{s.style.overflow="visible"};t.addEventListener("click",n),c.addEventListener("click",n),window.matchMedia("(min-width: 768px)").addEventListener("change",l=>{l.matches&&a()}),document.addEventListener("keydown",l=>{l.key==="Escape"&&e.classList.contains("is-open")&&a()})}J();const k=document.querySelector("[data-modal]"),W=document.querySelector("p[data-raiting]"),X=document.querySelector(".btn_close"),L=document.querySelector(".rating_form"),Y=[...document.getElementsByClassName("btn_star")];let f=0,g={},G="64f389465ae26083f39b17a2";X.addEventListener("click",e=>{e.preventDefault()});L.addEventListener("submit",e=>{if(e.preventDefault(),new FormData(e.currentTarget).forEach((s,n)=>{g[n]=s.trim()}),g.rate=f,g.rate<1){m.show({title:"The rate must be at least 1",color:"red",position:"topCenter",message:""});return}U(G,g)&&(m.show({title:"The rating has been successfully added",color:"green",position:"topCenter",message:""}),L.reset(),f=0,k.classList.add("is-hidden"))});function V(e){const t="btn_star-active",c="btn_star",s=e.length;let n;e.map(a=>{a.onclick=()=>{if(n=e.indexOf(a),a.className===c)for(n;n>=0;--n)e[n].className=t;else for(n;n<s;++n)e[n].className=c;f=e.filter(i=>{if(i.className===t)return i}).length,W.textContent=`${f}.0`}})}V(Y);(()=>{const e={openModalBtn:document.querySelector("[data-modal-open]"),closeModalBtn:document.querySelector("[data-modal-close]"),modal:document.querySelector("[data-modal]")};e.openModalBtn.addEventListener("click",t),e.closeModalBtn.addEventListener("click",t),e.modal.addEventListener("click",c);function t(){k.classList.contains("is-hidden")===!0&&document.addEventListener("keydown",s),e.modal.classList.toggle("is-hidden")}function c(n){n.target===n.currentTarget&&e.modal.classList.add("is-hidden")}function s(n){n.key==="Escape"&&(e.modal.classList.add("is-hidden"),document.removeEventListener("keydown",s))}})();const Z=document.querySelector(".cards"),ee=document.querySelector(".modal_window_update"),p=document.querySelector(".modal_window_container"),te=document.querySelector(".close_modal_button"),se=document.querySelector(".rate_button"),h=document.body,ne=document.querySelector(".rate_button"),ce=document.querySelector("[data-modal]");Z.addEventListener("click",re);te.addEventListener("click",ie);ne.addEventListener("click",oe);window.addEventListener("keydown",q);window.addEventListener("click",q);function ae({gifUrl:e,time:t,name:c,bodyPart:s,equipment:n,target:a,description:r,rating:i,burnedCalories:l,popularity:T}){return`<div class="modal_window_content">
...
</div >`}function oe(){ce.classList.remove("is-hidden"),p.classList.add("is-hidden")}function re(e){const t=e.target;if(!t.classList.contains("js-excercise-button"))return;const s=t.closest(".exercises-item").dataset.exerciseId;A(s).then(n=>{const a=ae(n);ee.innerHTML=a}),se.setAttribute("dataId",s),p.classList.remove("is-hidden"),h.style.overflow="hidden"}function q(e){e.key==="Escape"?(p.classList.add("is-hidden"),h.style.overflow="visible"):e.target.closest(".modal_window_default_content,.js-excercise-button")||(p.classList.add("is-hidden"),h.style.overflow="visible")}function ie(){p.classList.add("is-hidden"),h.style.overflow="visible"}const M="quote",le=document.querySelector(".quote-text"),de=document.querySelector(".quote-author");async function ue(){try{const{author:e,quote:t}=await O(),c=new Date().toLocaleDateString();localStorage.setItem(M,JSON.stringify({author:e,quote:t,date:c}))}catch(e){console.error("Failed to fetch the quote:",e)}}function $(){const e=localStorage.getItem(M);return JSON.parse(e)}function w(){const e=$();le.textContent=e.quote,de.textContent=e.author}async function me(){const e=new Date().toLocaleDateString(),t=$();if(t&&t.date===e){w();return}await ue(),w()}document.addEventListener("DOMContentLoaded",me);
